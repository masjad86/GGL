	...
	$.fn.extend({
		...
		function addEvent(event, setMonth, setYear) {
			...
			var startTime = _getEventDetail(event, "starttime"),
                ...
                isBooked = _getEventDetail(event, "isbooked"),
				dayStartTag = "<div",
				dayEndTags = "</span></div>";
			...
			var	markupDayStart = dayStartTag
				...
				markupListEvent = "<a"
                    ...
                    + (options.modalDialog.length > 0 && !isBooked ? attr("data-toggle", "modal") : "")
                    + (options.modalDialog.length > 0 && !isBooked ? attr("data-target", "#" + options.modalDialog) : "")
					+ attr("data-eventid", eventId)
		...
		$(document.body).on("click touchstart", parent + " .monthly-day", function (event) {
			...
			if(options.mode === "event" && options.eventList) {
				...
				var	theList = $(parent + " .monthly-event-list"),
                    ...
                    if(options.matchCellValue !== undefined && options.matchCellValue.length > 0 
                        && myElement.innerText.indexOf(options.matchCellValue) !== -1) {
                    } else {
                        theList.show();
